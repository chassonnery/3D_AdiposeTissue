...
Each movie displays the temporal evolution of a system composed of a dense network ($\phi_{fib} = 0.5$, i.e. $N_{fib} = 2578$) of long fibres ($L_{fib} = 6$) and a medium-sized population of cells ($\phi_{ad}=0.3$, i.e. $N_{ad}^{max} = 573$). The fibres are colored according to their alignment indicator, from blue ($Al_k=0$) to red ($Al_k=1$). Fibres with less than $40$ neighbours are colored in green. The cells are colored according to the index of the lobule to which they belong at the end of the simulation, with isolated cells in white.
