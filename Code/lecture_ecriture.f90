...
use definitions
	...
	read(15,*) Pmod%bias_ens, Pmod%Nseed_ens
	read(15,*) Pmod%Kgrowth
	read(15,*) temp
	...
	if (Pmod%Ncell.lt.0) then
		write(*,'(a)') "The initial number of cells must be a positive or null integer"
		STOP
	elseif (Pmod%Nmax.lt.Pmod%Ncell) then
		write(*,'(a)') "The maximum number of cells must be an integer greater than the initial number of cells"
		STOP
	elseif (Pmod%Rmin.le.0) then
		write(*,'(a)') "The minimum cell radius must be positive"
		STOP
	elseif (Pmod%Rmax.lt.Pmod%Rmin) then
		write(*,'(a)') "The maximum cell radius must be greater than the minimum cell radius"
		STOP
	elseif (Pmod%Nfib.lt.0) then
		write(*,'(a)') "The number of fibers must be a positive or null integer"
		STOP
	elseif (Pmod%Lf.lt.0) then
		write(*,'(a)') "The fiber length must be positive or null"
		STOP
	elseif (Pmod%Rf.le.0) then
		write(*,'(a)') "The fiber radius must be positive"
		STOP
	elseif ((Pmod%cumul_replink.ne.0).and.(Pmod%cumul_replink.ne.1)) then
		write(*,'(a)') "Parameter cumuk_replink can only be equal to 0 or 1"
		STOP
	elseif (Pmod%freq_ens.lt.0) then
		write(*,'(a)') "The temporal frequency for cell insemination must be positive or null"
		STOP
	elseif (Pmod%bias_ens.lt.0) then
		write(*,'(a)') "The spatial bias for cell insemination must be positive or null"
		STOP
	elseif (Pmod%Nseed_ens.lt.0) then
		write(*,'(a)') "The number of seeds for biased insemination must be positive or null"
		STOP
	elseif (Pmod%Kgrowth.lt.0) then
		write(*,'(a)') "The volumic growth rate of cells must be positive or null"
		STOP
	elseif (Pmod%freq_link.lt.0) then
		write(*,'(a)') "The fiber linking frequency must be positive or null"
		STOP
	elseif (Pmod%freq_dlink.lt.0) then
		write(*,'(a)') "The fiber unlinking frequency must be positive or null"
		STOP
	elseif ((Pmod%LinkType.ne.0).and.(Pmod%LinkType.ne.1).and.(Pmod%LinkType.ne.2)) then
		write(*,'(a)') "Parameter LinkType can only be equal to 0, 1 or 2"
		STOP
	elseif (Pmod%eps.lt.0) then
		write(*,'(a)') "The sensing distance for fiber linking must be positive or null"
		STOP
	elseif (Pmod%d0.lt.0) then
		write(*,'(a)') "The equilibrium length of the link springs must be positive or null"
		STOP
	elseif ((Pdom%xmaxR.le.0).or.(Pdom%ymaxR.le.0).or.(Pdom%zmaxR.le.0)) then
		write(*,'(a)') "The domain half-length in the three dimensions must be positive"
		STOP
	elseif (Pmod%DirFillingRate.lt.0) then
		write(*,'(a)') "The filling rate of the dirichlet layer must be positive or null"
		STOP
	elseif (Psim%Tf.le.0) then
		write(*,'(a)') "The final simulated time must be positive"
		STOP
	elseif (Psim%dt.le.0) then
		write(*,'(a)') "The maximum time-step allowed must be positive"
		STOP
	elseif ((Psim%InitType.ne.0).and.(Psim%InitType.ne.1).and.(Psim%InitType.ne.2)) then
		write(*,'(a)') "Parameter InitType can only be equal to 0, 1 or 2"
		STOP
	elseif (Psim%Tenr.le.0) then
		write(*,'(a)') "The data saving frequency must be positive"
		STOP
	end if
...
use fonctions
	...
	link = 0
	distpoints = 0.0
	if ((Pmod%LinkType.ne.2).and.(Pmod%Nfib.ne.0)) then
...
use definitions
	...
	do k = Nsave0,Nsavef
		...
		! Links --------------------------------------------------------------------------------------
		if ((Pmod%Nfib.ne.0).and.(Pmod%LinkType.eq.0)) then 
...
subroutine WriteInput(Xcell,Rcell,Xfib,omega,link,distpoints,Pmod,Psim)
use definitions
	implicit none
	double precision, dimension(:,:), intent(inout) :: Xcell, Xfib, omega, distpoints
	double precision, dimension(:), intent(inout)   :: Rcell
