...
!----------------------------------------------------UTILITAIRES---------------------------------------------------------!
double precision, dimension(:,:), allocatable :: Xcell, Xfib, omega, distpoints, gradC, gradF
...
double precision                              :: tcpu0,tcpuf,tcpuglob0,tcpuglobf,tcpu_write,t_end_ins
integer                                       :: tcpu_days,tcpu_hours,tcpu_secs
!----------------------------------------------------REDEMARRAGE SIM-----------------------------------------------------!
character(len=200)                            :: nomdossier_old
integer                                       :: NbStepInPreviousSim
!----------------------------------------------------ENREGISTREMENT DONNEES----------------------------------------------!
type(SaveState)                               :: Saving
integer, parameter                            :: Nmemorymax = 10 ! 1000
...
if (Pdom%Nx.le.1) then
	write(*,'(a)') "ATTENTION : Nx < 2 !"
	write(*,'(a)') "  Le nombre de boîtes numériques est insuffisant pour assurer le bon fonctionnement des conditions aux limites."
	write(*,'(a)') "  Veuilliez augmenter la taille Lx du domaine ou réduire la distance maximale d'interaction."
	STOP
else if (Pdom%Ny.le.1) then
	write(*,'(a)') "ATTENTION : Ny < 2 !"
	write(*,'(a)') "  Le nombre de boîtes numériques est insuffisant pour assurer le bon fonctionnement des conditions aux limites."
	write(*,'(a)') "  Veuilliez augmenter la taille Ly du domaine ou réduire la distance maximale d'interaction."
	STOP
else if (Pdom%Nz.le.1) then
	write(*,'(a)') "ATTENTION : Nz < 2 !"
	write(*,'(a)') "  Le nombre de boîtes numériques est insuffisant pour assurer le bon fonctionnement des conditions aux limites."
	write(*,'(a)') "  Veuilliez augmenter la taille Lz du domaine ou réduire la distance maximale d'interaction."
	STOP
end if 
...
!------- Methode PIC --------------------------------------------------------------------------------------
dist = 2*Pmod%Rmax ! minimal size of a box > maximal radius of an agent

Pdom%NxPIC = int(Pdom%xmaxR*1.0/dist) ! number of boxes that can fit in the dimension x
Pdom%NyPIC = int(Pdom%ymaxR*1.0/dist)
Pdom%NzPIC = int(Pdom%zmaxR*1.0/dist)

Pdom%dxPIC = Pdom%xmaxR*1.0/Pdom%NxPIC ! size of the boxes in the dimension x
Pdom%dyPIC = Pdom%ymaxR*1.0/Pdom%NyPIC
Pdom%dzPIC = Pdom%zmaxR*1.0/Pdom%NzPIC
...
write(101,'(a)') "# time, real elapsed time, internal links, external links, Ncell, number of step since previous saving"
close(101)
...
if (Psim%InitType==0) then
	...
	call WriteInput(Xcell,Rcell,Xfib,omega,link,distpoints,Pmod,Psim)
...
if (Pmod%Ndirtot.ne.0) then
	...
	write(2,'(a,i0)') "# Number of Dirichlet's fibers per box : ", int(Pmod%DirFillingRate * Pdom%dx*Pdom%dy*Pdom%dz)
	write(2,'(a,i0)') "# Total number of Dirichlet's fibers : ", Pmod%Ndirtot
	do i=Pmod%Nfib+1,Pmod%Nfib+Pmod%Ndirtot
		write(2,'(6(e23.17e2,1x))') Xfib(i,1), Xfib(i,2), Xfib(i,3), omega(i,1), omega(i,2), omega(i,3)
	end do
	close(2)
	
	write(*,'(a,i0)') "Total number of Dirichlet's fibers : ", Pmod%Ndirtot
end if
			...
			do l2 = -1,1,1
				do l3 = -1,1,1
					...
					if ( (ltemp.lt.1).or.(ltemp.gt.8*(Pdom%Nx+1)*(Pdom%Ny+1)*(Pdom%Nz+1)) ) then
						write(*,'(a,f0.5,a)') "BoxCell number is out of range at motion-step (time = ", time, ")."
						write(*,'(6(a,i0),a)') "Cell ", i, " is in box ", l, " (with l1 =", l1, " l2 =", l2,&
											   " l3 =", l3, " and ltemp = ", ltemp, ")."
						write(*,'(a,3(f0.5,1x))') "Its coordinates are ", Xcell(i,:)
						STOP
					end if
		...
		do l1 = -1,1,1 ! for each neighbor k of fiber i, compute fiber-fiber interaction
			do l2 = -1,1,1
				do l3 = -1,1,1
					...
					if ( (ltemp.lt.1).or.(ltemp.gt.8*(Pdom%Nx+1)*(Pdom%Ny+1)*(Pdom%Nz+1)) ) then
						write(*,'(a,f0.5,a)') "BoxFib number is out of range at motion-step : time = ", time
						write(*,'(6(a,i0),a)') "Fiber ", i, " is in box ", l, " (with l1 =", l1, " l2 =", l2,&
											   " l3 =", l3, " and ltemp = ", ltemp, ")."
						write(*,'(a,3(f0.5,1x))') "Its coordinates are ", Xfib(i,:)
						STOP
					end if
...
!------------------------------- SAVE CURRENT STATE TO RAM --------------------------------------------
	if (time.ge.(Nsavestep + Nsaveblock*Nmemorymax)*Psim%Tenr) then
		...
		Nlinks_int = sum(link(:,1:Pmod%Nfib))/2
		...
		call cpu_time(tcpuf)
		tcpu_write = tcpu_write + tcpuf - tcpu0
		call cpu_time(tcpu0)
	endif
...
!---------------------------------- CELL GROWTH -------------------------------------------------------
	do i = 1,Pmod%Ncell
		Rcell(i) = (Rcell(i)**3 + dt*Pmod%Kgrowth)**(1./3.)
...
!----------------------------- CELL INSEMINATION ------------------------------------------------------
	if (Pmod%Ncell < Pmod%Nmax) then
		if (Pmod%Ncell<Pmod%Nseed_ens) then
			call insemination_unif(Xcell,Rcell,dt,time,Pmod,Pdom)
		else
			call insemination_biased(Xcell,Rcell,dt,time,Pmod,Pdom)
		end if
...
!---------------------------- CLEAN BOXES AND REATTRIBUTE ---------------------------------------------
	...
	call cleanInsideDomain(Pdom,BoxFib)
	do i = 1,Pmod%Nfib
			...
			do l1 = -1,1,1 ! search all neighbors k of fiber i
				do l2 = -1,1,1
					do l3 = -1,1,1
						...
						if ( (ltemp.lt.1).or.(ltemp.gt.8*(Pdom%Nx+1)*(Pdom%Ny+1)*(Pdom%Nz+1)) ) then
							write(*,'(a,f0.5,a)') "BoxFib number is out of range at linking-step : time = ", time
							write(*,'(6(a,i0),a)') "Fiber ", i, " is in box ", l, " ( with l1 =", l1, " l2 =", l2,&
												   " l3 =", l3, " and ltemp = ", ltemp, ")"
							write(*,'(a,3(f0.5,1x))') "its coordinates are ", Xfib(i,:)
							STOP
...
!------------------------------ FINALISATION ----------------------------------------------------------------------
!------------------------------------------------------------------------------------------------------------------
call cpu_time(tcpuglobf)
tcpu_days  = floor( (tcpuglobf - tcpuglob0)/86400 )
tcpu_hours = floor( (tcpuglobf - tcpuglob0)/3600 - tcpu_days*24 )
tcpu_secs  = floor( tcpuglobf - tcpuglob0 - tcpu_days*86400 - tcpu_hours*3600)
...
write(101,*)
write(101,*)
write(101,'(a,3(i0,a))') "# Total time of computation : ",tcpu_days," days, ",tcpu_hours," hours and ",tcpu_secs," seconds."
write(101,'(a,f0.4)') "# Total time of computation (in seconds) : ", tcpuglobf - tcpuglob0
call cpu_time(tcpuf)
write(101,'(a,f0.4)') "# Time needed to write data (in seconds) : ", tcpu_write + tcpuf - tcpuglobf
write(101,'(a,f0.4)') "# Internal time of the simulation at the end of the insemination process : ", t_end_ins
close(101)
